...
def _make_bn3_part(ncp_dict: Dict[str, Any], internal_id: int) -> List[NaviCustPart]:
    ...
    return [
        NaviCustPart(
            name, BN3NaviCustPartColor[color], description, compression_code, bug, layout, internal_id + offset
        )
...
ILLEGAL_PARTS = ncp_list_utils.get_unobtainable_parts(GAME, _make_bn3_part)
